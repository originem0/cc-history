...
- **会话浏览** — 按项目分组展示所有 Claude Code 会话，项目按最近活跃排序且默认展开；每条会话显示相对时间与消息数，侧边栏树形导航
- **智能标题** — 自动从首条实质用户消息提取标题，跳过命令回显、skill 注入、纯寒暄/数字；无实质内容时回退为「项目名 · 日期」
...
- **加载串行化** — `Load()` 使用独立互斥锁串行执行，防止文件监控与手动刷新并发交错导致项目计数错乱
- **LRU 缓存 3 条对话** — 避免反复解析大 JSONL 文件，仅在对应文件变化时失效
- **JSONL 逐行容错** — 跳过损坏行，不崩溃（Claude Code 可能正在写入）
- **绑定 127.0.0.1 + Host/Origin 校验** — 禁止局域网访问；校验请求 Host 与 Origin 必须为 loopback，阻断 DNS rebinding（恶意网页将域名重绑到 127.0.0.1 后即可窃取全部会话历史，或借 MCP 配置写入实现 RCE）
...
- **命令注入防护** — 会话恢复前验证 session ID 格式（UUID hex 模式），阻止恶意输入；工作目录通过 `cmd.Dir` 传递而非拼接进命令行，避免 cmd 元字符（`& | ^ %`）注入
- **智能标题提取** — 跳过 `/` 命令、命令回显（`<local-command-stdout>` 等）、skill 注入、XML 系统提示、纯寒暄/单字符/数字菜单回答，取首条实质消息；无则回退「项目名 · 日期」
- **搜索先排序后截断** — 收集全部匹配后按时间倒序再截断到 limit，避免在匹配数超限时遗漏最新结果
